<template>
    <form @submit.prevent="submit">
        ...
        <p class="mt-3 mb-4">{{ useTrans('step1')?.page_title }}</p>

        <div class="mb-3">
            <label for="name" class="form-label">{{ useTrans('step1')?.project_name }}</label>
            <input  type="text" name="project_name" class="form-control" id="name" v-model="$store.state.step1.project_name" maxlength="100">
            <small class="float-end">{{ $store.state.step1.project_name.length }}/{{ useTrans('step1')?.caracter_limit }}</small>
        </div>

        <p class="fw-bold">{{ useTrans('step1')?.project_type }}</p>
        <p>{{ useTrans('step1')?.project_type_description }}</p>
        <hr>
        ...
        <div class="form-check">
            ...
            <label class="form-check-label" for="check_type1">
                {{ useTrans('step1')?.humanitarian }}
            </label>
            <small>{{ useTrans('step1')?.humanitarian_description }}</small>
        </div>
        ...
        <div class="form-check">
            ...
            <small>{{ useTrans('step1')?.team_description }}</small>
        </div>
        ...
        <div class="form-check">
            ...
            <small>{{ useTrans('step1')?.beca_description }}</small>
        </div>

        <hr>

        <p class="fw-bold">{{ useTrans('step1')?.contacts }} <span class="float-end"><i class="fa-regular fa-circle-question"></i></span></p>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th scope="col">{{ useTrans('step1')?.contact_patro }}</th>
            ...
            <tbody>
                ...
                <tr>
                    <td ><a href="#!" @click="showModal()" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true" data-bs-toggle="modal" data-bs-target="#exampleModal" >+ {{ useTrans('step1')?.add_contact }}</a></td>
                </tr>
            </tbody>
        </table>
            ...
            <div class="modal-dialog modal-dialog-scrollable modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <h3 class="mb-3">{{ useTrans('step1')?.modal?.title }}</h3>
                        <h6 class="text-uppercase">{{ useTrans('step1')?.modal?.first_contact }}</h6>
                        <small>{{ useTrans('step1')?.modal?.default }}</small>
                        <hr>

                        <div class="mb-3">
                            <label for="name" class="form-label">{{ useTrans('step1')?.modal?.role }}</label>

                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="contacto1_papel" id="local" value="Local" v-model="prin_contact.papel">
                                <label class="form-check-label" for="local">{{ useTrans('step1')?.modal?.local }}</label>
                            </div>
                            <div class="form-check form-check-inline">
                                ...
                                <label class="form-check-label" for="internacional">{{ useTrans('step1')?.modal?.international }}</label>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label for="name" class="form-label">{{ useTrans('step1')?.modal?.your_patro }}</label>
                            ...
                            <div class="form-check form-check-inline">
                                ...
                                <label class="form-check-label" for="patrocinador_local">{{ useTrans('step1')?.modal?.local }}</label>
                            </div>
                            <div class="form-check form-check-inline">
                                ...
                                <label class="form-check-label" for="patrocinador_internacional">{{ useTrans('step1')?.modal?.international }}</label>
                            </div>
                        </div>


                        <h6 class="text-uppercase">{{ useTrans('step1')?.modal?.second_contact }}</h6>
                        <small>{{ useTrans('step1')?.modal?.second_contact_description }}</small>
                        <hr>

                        <div class="mb-3">
                            <label for="name" class="form-label">{{ useTrans('step1')?.modal?.his_patro }}</label>
                            ...
                            <div class="form-check form-check-inline">
                                ...
                                <label class="form-check-label" for="distrito">{{ useTrans('step1')?.modal?.district }}</label>
                            </div>
                            <div class="form-check form-check-inline">
                                ...
                                <label class="form-check-label" for="rotarac_club">{{ useTrans('step1')?.modal?.rotarac_club }}</label>
                            </div>
                            <div class="form-check form-check-inline">
                                ...
                                <label class="form-check-label" for="rotary_club">{{ useTrans('step1')?.modal?.rotary_club }}</label>
                            </div>
                        </div>
                        ...
                        <table class="display mt-5" width="100%">
                            <thead>
                                <tr>
                                    <th>{{ useTrans('step1')?.modal?.member_number }}</th>
                                    <th>{{ useTrans('step1')?.modal?.names }}</th>
                                    <th>{{ useTrans('step1')?.modal?.last_names }}</th>
                                    <th>{{ secun_contact.names.length }} {{ useTrans('step1')?.modal?.selected }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(item, index) in step1_contacts" :key="index">
                                    ...
                                    <td>{{ item.lastname }}</td>
                                    <td>
                                        <button class="btn btn-outline-primary my-2" :class="(this.secun_contact.names.indexOf(item) === -1) ? 'btn-outline-primary' : 'btn-primary text-white' " @click="addContactName(item)">{{ useTrans('step1')?.modal?.select }}</button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" @click="addContacts()" data-bs-dismiss="modal">{{ useTrans('layout')?.save }}</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    data() {
        return {
            prin_contact: {
                patrocinador: useTrans('step1')?.modal?.international,
                papel: useTrans('step1')?.modal?.local,
            },
            secun_contact: {
                club: 'New York',
                district: '6990',
                patrocinador: useTrans('step1')?.modal?.local,
                papel: useTrans('step1')?.modal?.local,
                names: [],
            },
            clubs: clubs,
    ...
    methods: {
        ...
        addContacts() {
            ...
            this.secun_contact.names.forEach((el, i) => {
                this.$store.state.step1.contacts?.push({
                    ...
                    district: '6990',
                    patrocinador: useTrans('step1')?.modal?.international,
                    papel: useTrans('step1')?.modal?.international,
                })
            })
        ...
        submit() {
            ...
            .then((response) => {
                ...
                this.$swal(useTrans('step1')?.success_message, '', 'success');
